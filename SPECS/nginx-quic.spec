...
EXCC_OPTS="-mcpu=native -ftree-vectorize -fuse-linker-plugin -fuse-ld=gold -fopenmp -fPIC"
CFLAGS="$(echo %{optflags} $(pcre-config --cflags))"
CFLAGS="${CFLAGS} ${EXCC_OPTS}"; export CFLAGS;
export CXXFLAGS="${CFLAGS}"
LDFLAGS="%{?__global_ldflags} $(pcre-config --libs) -pie --as-needed -lslz"
export LDFLAGS;
...
./auto/configure \
  ...
  --with-openssl-opt="enable-ktls zlib -O3 -DOPENSSL_USE_NODELETE -DOPENSSL_PIC -shared" \
  --prefix=%{nginx_home} \
